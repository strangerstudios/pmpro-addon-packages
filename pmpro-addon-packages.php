...
function pmproap_post_meta()
{
    ...
    ?>
    <input type="hidden" name="pmproap_noncename" id="pmproap_noncename"
           value="<?php echo wp_create_nonce(plugin_basename(__FILE__)) ?>"/>

    <?php if ($pmproap_price && empty($pmpro_page_levels[$post->ID])) { ?>
    <p><strong class="pmpro_red">Warning: This page is not locked down yet.</strong> You must select at least one
        membership level in the sidebar to the right to restrict access to this page. You can create a free membership
        level for this purpose if you need to.</p>
<?php } elseif ($pmproap_price) { ?>
    <p><strong class="pmpro_green">This page is restricted.</strong> Members will have to
        pay <?php echo pmpro_formatPrice($pmproap_price); ?> to gain access to this page. To open access to all members,
        delete the price below then Save/Update this post.</p>
<?php } else { ?>
    <p>To charge for access to this post and any subpages, set a price below then Save/Update this post. Only members of
        the levels set in the "Require Membership" sidebar will be able to purchase access to this post.</p>
<?php } ?>

    <div>
        <label><strong>Price</strong></label>
        &nbsp;&nbsp;&nbsp; <?php echo $pmpro_currency_symbol; ?><input type="text" id="pmproap_price"
                                                                       name="pmproap_price"
                                                                       value="<?php echo esc_attr($pmproap_price); ?>"/>
    </div>
    <?php
}
...
function pmproap_profile_fields($user_id)
{
    ...
    <table class="form-table">
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <span id="pmproap_remove_span_<?php echo $upost->ID; ?>">
							<a target="_blank"
                               href="<?php echo esc_attr(get_permalink($upost->ID)); ?>"><?php echo $upost->post_title; ?></a>
							&nbsp; <a style="color: red;" id="pmproap_remove_<?php echo $upost->ID; ?>"
                                      class="pmproap_remove" href="javascript:void(0);">remove</a>
							</span>
                        <?php
                        ?>
                    </td>
        ...
        <tr>
            ...
            <td>
                <input type="text" id="new_pmproap_posts_1" name="new_pmproap_posts[]" size="10" value=""/>
                <small>Enter a post/page ID</small>
            </td>
        </tr>
