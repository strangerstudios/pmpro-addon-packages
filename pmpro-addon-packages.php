        ...
        if (pmproap_isPostLocked($post->ID) && !pmproap_hasAccess($current_user->ID, $post->ID)) {
			...
			if(empty($text_level_id)) {
				...
				if(!empty($all_access_levels))
				{
					...
					$text = "<p>" . sprintf ( __( "This content requires that you purchase additional access. The price is %s or free for our %s members.", "pmproap"), pmpro_formatPrice($pmproap_price), pmpro_implodeToEnglish($level_names) ) . "</p>";
